    ...
    if A.shape[-1] > proj_dims:
        if method == "umap":
            proj_data = umap.UMAP(
                ...
                metric="correlation",
                **kwargs,
            ).fit_transform(A.cpu().numpy())
            proj_data = torch.from_numpy(proj_data).to(tokens.device)
        else:  # pca
            (U, S, V) = torch.pca_lowrank(A)
            proj_data = torch.matmul(
                A, V[:, :proj_dims]
            )  # this is the actual PCA projection step
    else:
        proj_data = A
    if debug:
...
def scatter3d(x_sum, x_mix, xs=None, method="pca", prefix="z", debug=False):
    ...
    for i in range(len(labels)):
        ...
        if debug:
            print("dat_3d.shape =", dat_3d.shape)
...
@torch.no_grad()
def viz_sums_mixes(
    x_sum, x_mix, xs=None, method="pca", prefix="z", debug=False, sum_fac=1.0
):
    if debug:
