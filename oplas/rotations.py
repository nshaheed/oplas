...
def solve_rotation_ap(u, v, check_vecs=False, debug=True):
    ...
    assert len(u.shape) == 1, (
        f"u ({list(u.shape)}) & v ({list(v.shape)}) should be single vectors, not batches"
    )
...
class FiLMRnd(nn.Module):
    ...
    def __init__(
        ...
        self.v = self.u + uv_diff_fac * torch.randn(
            (rank_uv, nd)
        )  # make v a bit different from u
        if self.norm_uv:
    ...
    def forward(self, x, debug=False):
        ...
        if debug:
            print(
                "self.u.shape, self.v.shape, rot.shape =",
                self.u.shape,
                self.v.shape,
                rot.shape,
            )
