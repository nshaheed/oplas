...
def vicreg_loss_fn(
    ...
    lambda_v=1e-5,
    lambda_i=1.0,
    lambda_c=0.1,  # regularization/tuning parameters
...
def mag_loss_fn(
    z,
    lambda_m: float = 0.01,  # scale factor
):
    ...
    return (
        lambda_m * (phi - 1).pow(2).mean()
    )  # simpler version, just a parabola around phi=1
...
def get_loss_on_batch(
    ...
    if True:
        ...
        mag_loss = torch.stack(
            [mag_loss_fn(z) for z in [z_mix, z_sum]]
        ).mean()  # keep z's, zsum, mix  mag's kinda near 1
